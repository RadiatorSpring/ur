...
function createGame(name, pass) {
	...
	xhr.addEventListener("readystatechange", function () {
		if (this.readyState === 4) {
			...
			if (this.status != 200) {
				alert("There was problem with login")
			}
			else {
				isAuth = true;
			}
...
function registerUser() {
	...
	let data = JSON.stringify({ "username": name, "password": pass });
	...
	xhr.addEventListener("readystatechange", function () {
		if (this.readyState === 4) {
			console.log(this.responseText);
			if (this.status != 200) {
				alert("There was problem with user creation")
			}
			if (this.status == 200) {
	...
	xhr.withCredentials = false;
	xhr.open("POST", "http://localhost:8080/murabi/v1/register", false);
	xhr.setRequestHeader("Content-Type", "application/json");
	xhr.send(data);
...
function startGame() {
	...
	let createResp = createGame(nick, pass)
	console.log("The resp is " + createResp)
	if (createResp != "") {
